...
class _CustomersPageState extends State<CustomersPage> {
  ...
  Future<void> _refresh({bool clear = false}) async {
    ...
    try {
      ...
      // Resumo (ativos/inativos)
      final active = await _repo.search(q: '', limit: 1, skip: 0, onlyActive: true);
      final inactive = await _repo.search(q: '', limit: 1, skip: 0, onlyActive: false);
      setState(() {
        _countActive = active.length; // não é 100% preciso com paginação, mas dá feedback rápido
        _countInactive = 0; // sem endpoint dedicado; opcional: criar cloud function para contagem
      });
    } catch (_) {
      // ignore / snackbar adiante se quiser
    } finally {
                    ...
                    Expanded(
                      child: _loading && _items.isEmpty
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return _CustomerRow(
                            c: c,
                            onEdit: () => context.push('/clientes/editar/${c.objectId}'),
