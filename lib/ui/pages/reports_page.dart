...
class _ReportsPageState extends State<ReportsPage> {
  ...
  Future<void> _loadFinance() async {
    ...
    // filtro por método (se não "ALL")
    final filterAll = _payFilter.contains('ALL');
    for (final o in rows) {
      final id = o.objectId ?? '';
      final dt = o.createdAt ?? DateTime.now();
      final method = (o.get<String>('paymentMethod') ?? 'OTHER').toUpperCase();
      if (!filterAll && !_payFilter.contains(method)) continue;

      // tenta pegar os números com fallback
      final gross = (o.get<num>('gross') ?? o.get<num>('total') ?? 0).toDouble();
      final disc = (o.get<num>('discount') ?? 0).toDouble();
      final net = (o.get<num>('net') ?? (gross - disc)).toDouble();
      final received = (o.get<num>('received') ?? net).toDouble();
      final change = (o.get<num>('change') ?? 0).toDouble();
